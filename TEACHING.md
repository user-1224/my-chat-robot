...
## 1、使用技术栈
- 前端：HTML + TypeScript
- 接口请求：fetch 网络请求调用AI绘图接口
- 本地存储：localStorage 存储历史记录

## 2、核心模块说明
1. 绘图请求模块
编写异步函数，向AI图片接口发送POST请求，传递提示词、模型参数，接收返回的图片地址。

2. 页面渲染模块
将接口返回的图片URL渲染到页面DOM中，展示图片内容。

3. 文件下载模块
创建下载链接，实现图片下载到本地浏览器。

4. 历史记录模块
把每一次的提示词和图片地址存入本地存储，刷新页面后依然可以查看历史记录。

## 3、开发中遇到的问题与解决
1. 跨域请求限制：了解后端代理转发的解决方案
2. 异步加载顺序问题：使用async/await控制请求先后顺序
3. 刷新页面数据丢失：使用浏览器本地持久化存储localStorage解决
